...
## Coherence pass — reading both series as one argument

A pass over all eleven posts for places where a newer, more careful position
had been added and an older, looser sentence left standing, or where a
unifying idea was used everywhere and stated nowhere.

- **The single principle is now named.** Both series make one move — the work's real structure is the source of truth and every human shape is a view rendered from it — and used it in Part 4 (roles as projection), Layer 4 (audit trail as view), The Data Shapes (tables, pages, inboxes as views), and The Ones That Survive (release notes at the boundary) without ever saying so. It is now stated in START-HERE, both READMEs, the opening of *Leaving the Cage*, and the close of *What "AI-First" Actually Means*.
- **"One trap, the same six" is narrowed to what it covers.** The six explain the coordination tools directly; the data shapes lean on the shipped primitive and on perception, which the six do not cover. Stated in Post 0 (both copies), Post 4, the Prometheus README, and applied as step five of the method at the end of *The Data Shapes* and *The Ones That Survive*. The Rituals verdict now maps its fourth reason (easier to explain to a new hire) to the readable trace.
- **The Part 1 table is brought to the current position.** It no longer says the model holds everything in context, that self-critique is sufficient, or that shared state is unconditional. Review and testing are marked as rationing an independent second look rather than attention, and the origin section's claim that every row is an attention limit now says all but two.
- **Correlated error is absorbed, not annotated.** Part 1 states the bet explicitly (shared cognition is the claim, correlated error the price, the mitigations the wager). Property 3 and the perspective-stitched description no longer contradict "sync late." Part 5's recommendation to start with multi-voice now carries the blind-first-round instruction. The two Part 3 ablations are presented as one two-by-two. *The Rituals* review verdict now says the independent second reader survives and moves from a person with less context to a model with a different one. *Leaving the Cage* gains a first move, "form first positions blind."
- **Overclaims trimmed.** "The limitation is not technical" is now "the ceiling is not technical," with the technical problems named as problems of degree. START-HERE presents the sixty-cents figure as the worked illustration it is, and joins the three cost stories (hand-off loss, wrong scarcity, translation tax) to their one cause. The capstone notes that the reasoning layer rests on measured work and the tool layer on argument.
- **The counts agree.** Part 1's prose now matches its table (the primitive buys convenience; two causes, not three, were never chosen; the word *agent* was chosen, its consequences were not). Part 2 no longer claims each path treats exactly one cause when its own table shows otherwise.
- **The sub-agent specimen is argued, not asserted.** Compression is not the mistake — context is scarce. The mistake is that the sender chose in advance (ownership) when the reader could have selected per question (relevance). Stated in Part 1 and START-HERE.
- **The MoE aside is motivated** as an objection a reader would raise, and its interpretability claim is softened to what can be said without a citation.
